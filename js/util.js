function moveNext(obj, len, nextId) {   // 현재엘레멘트, 몇자리까지?, 옮겨갈 엘레멘트
...
function checkNumber(event) {
    ...
    if ((key >= 48 && key <= 57) || (key >= 96 && key <= 105) || key == 9 || key == 8 || key == 13 || key == 37 || key == 39 || key == 46 || key == 109 || key == 189) {
        return true;  // 숫자키와 지우기 같은 기능키만 허용
    } else {
        if (window.event) {
...
function checkEmail(obj){
    var regexp  = /^([a-zA-Z0-9_\-\.\+]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([a-zA-Z0-9\-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/;  //정규식
